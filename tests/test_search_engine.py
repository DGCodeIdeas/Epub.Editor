...
class TestSearchEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_book.manifest = {
            ...
            'item2': ManifestItem(id='item2', href='content/page2.xhtml', media_type='application/xhtml+xml'),
            'item3': ManifestItem(id='item3', href='styles/style.css', media_type='text/css'),
        }
        ...
        self.page1_content = b'<html><body><p>This is a sample page with the word Test.</p></body></html>'
        self.page2_content = b'<html><body><p>Another page, without the keyword.</p></body></html>'
    ...
    def test_simple_search(self):
        ...
        results = list(self.search_engine.search(query='test', case_sensitive=False, whole_word=True, regex=False))
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].match_text, 'Test')

    def test_case_sensitive_search(self):
        """Test a case-sensitive search."""
        results = list(self.search_engine.search(query='Test', case_sensitive=True, whole_word=True, regex=False))
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].match_text, 'Test')

        results_lower = list(self.search_engine.search(query='test', case_sensitive=True, whole_word=True, regex=False))
        self.assertEqual(len(results_lower), 0)

    def test_whole_word_search(self):
        """Test the whole word search functionality."""
        results = list(self.search_engine.search(query='sam', case_sensitive=False, whole_word=True, regex=False))
        self.assertEqual(len(results), 0)

        results_full = list(self.search_engine.search(query='sample', case_sensitive=False, whole_word=True, regex=False))
        self.assertEqual(len(results_full), 1)

    def test_regex_search(self):
        """Test a search using a regular expression."""
        results = list(self.search_engine.search(query=r'T..t', case_sensitive=True, whole_word=False, regex=True))
        # Should find "Test"
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].match_text, 'Test')
    ...
    def test_no_results(self):
        ...
        results = list(self.search_engine.search(query='nonexistent', case_sensitive=False, whole_word=False, regex=False))
        self.assertEqual(len(results), 0)
